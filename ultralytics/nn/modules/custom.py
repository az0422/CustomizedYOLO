...
class DetectorTinyv5(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.cv1 = nn.ModuleList(nn.Sequential(Conv(x, c1, 1, 1), Conv(c1, c1, 3, 1, None, c1, 1), Conv(c1, c1, 3, 1, None, c1, 1)) for x in ch)
    ...
    def bias_init(self):
        ...
        for a, s in zip(m.cv2, m.stride):  # from
            a.bias.data[:m.reg_max * 4] = 1.0  # box
            a.bias.data[m.reg_max * 4:] = math.log(5 / m.nc / (640 / s) ** 2)
